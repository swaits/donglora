...
fuzz_target!(|data: &[u8]| {
    ...
    let max_enc = donglora_cobs::max_encoded_len(data.len()) + 1;
    let mut enc_buf = vec![0u8; max_enc];

    let enc_len = match donglora_cobs::encode(data, &mut enc_buf) {
        Some(n) => n,
    ...
    let dec_len = donglora_cobs::decode(encoded, &mut dec_buf)
